...
{
    if (__unlikely((*((uintptr_t*)bnd-1) & 1) == 1 && (jl_typetagof(val)->gc_bits & 1) == 0))
        gc_queue_binding(bnd);
}

static inline void gc_wb(void *parent, void *ptr) // parent and ptr isa jl_value_t*
{
    if (__unlikely((jl_typetagof(parent)->gc_bits & 1) == 1 &&
                   (jl_typetagof(ptr)->gc_bits & 1) == 0))
...
static inline void gc_wb_buf(void *parent, void *bufptr) // parent isa jl_value_t*
{
    ...
    if (__unlikely((jl_typetagof(parent)->gc_bits & 1) == 1))
        ...
        gc_setmark_buf(bufptr, jl_typetagof(parent)->gc_bits);
}

static inline void gc_wb_back(void *ptr) // ptr isa jl_value_t*
{
    // if ptr is marked
    if(__unlikely((jl_typetagof(ptr)->gc_bits & 1) == 1)) {
