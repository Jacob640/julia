...
{
    size_t sz = sizeof(jl_typetag_t) + 1 * sizeof(void*);
#ifdef MEMDEBUG
    return jl_valueof(alloc_big(sz));
#endif
#ifdef _P64
    return jl_valueof(_pool_alloc(&pools[2], sz));
#else
    return jl_valueof(_pool_alloc(&pools[0], sz));
#endif
}

DLLEXPORT jl_value_t *alloc_2w(void)
{
    size_t sz = sizeof(jl_typetag_t) + 2 * sizeof(void*);
#ifdef MEMDEBUG
    return jl_valueof(alloc_big(sz));
#endif
#ifdef _P64
    return jl_valueof(_pool_alloc(&pools[4], sz));
#else
    return jl_valueof(_pool_alloc(&pools[1], sz));
#endif
...
{
    size_t sz = sizeof(jl_typetag_t) + 3 * sizeof(void*);
#ifdef MEMDEBUG
    return jl_valueof(alloc_big(sz));
#endif
#ifdef _P64
    return jl_valueof(_pool_alloc(&pools[6], sz));
#else
    return jl_valueof(_pool_alloc(&pools[2], sz));
#endif
}
